...
public class StructurefulPlugin implements IModPlugin {
    ...
    public ResourceLocation getPluginUid() {
        return ResourceLocation.fromNamespaceAndPath(Structureful.MODID, "jei_plugin");
    }
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        if (recipeManager != null){
            List<AdvancedFurnaceRecipe> abstractCookingRecipes = new ArrayList<>();

            abstractCookingRecipes.addAll(
                    Minecraft.getInstance().level.getRecipeManager()
                            .getAllRecipesFor(AdvancedFurnaceBlockEntity.getRecipeType())
                            .stream()
                            .map( r -> r.value())
                            .toList()
            );
            registration.addRecipes(ADVANCED_FURNACE_TYPE, abstractCookingRecipes);
        }
    }
